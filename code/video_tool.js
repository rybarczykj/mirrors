...
let BIGGEST_PIXEL = 40;
let SMALLEST_PIXEL = 10;
...
function setup() {
  ...
  capture = createVideo('../assets/sprint_shorten.mp4');
  capture.volume(0);
...
function draw() {
  ...
  let bubbleSize = floor(map(mouseX, 0, width, SMALLEST_PIXEL, BIGGEST_PIXEL));
  ...
  for (var starty = 0; starty < height; starty++) {
    for (var startx = 0; startx < width; startx++) {
      ...
      // ellipse(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      random = Math.floor(Math.random() * 2) == 0;

      // rect(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      ellipse(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      // rect(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      // ellipse(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      // rect(x_loc, y_loc, bubbleSize * circle_ratio, bubbleSize * circle_ratio);
      // triangle(startx, starty, startx + (bubbleSize / 2), starty + bubbleSize, startx + bubbleSize, starty) // upside down triangle
      // triangle(startx, starty, startx, starty + bubbleSize, startx + bubbleSize, starty)

      startx = startx + bubbleSize - 1;
    }
    starty = starty + bubbleSize - 1;
  }
  setTimeout(1);
}
